...
function ForceErr
{
    ...
    Copy-Item -Path "C:\Windows\Temp\Logs\Bios\Lenovo_Bios_Config.log" -Destination "C:\Windows\Temp\Logs\Bios\Lenovo_Bios_Config.nok" -Force
...
if ($Manufacturer -eq 'LENOVO')
{
    ...
    foreach ($item in $MBCPSettings)
    {
        ...
        foreach ($item1 in $BiosSettings)
        {
            ...
            if ($Setting -eq $CpuSetting)
            {
                ...
                {
                    $needsave = $true
                    $setvalue = (Get-WmiObject -class Lenovo_SetBiosSetting –namespace root\wmi).SetBiosSetting("$Setting,$value,$LenovePar1,ascii,us")
    ...
    if ($needsave -ne $null)
    {
        $Save = (Get-WmiObject -class Lenovo_SaveBiosSettings -namespace root\wmi).SaveBiosSettings("$LenovePar1,ascii,us”)  
